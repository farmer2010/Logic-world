    ...
    def update(self, data={}, enr=1):
        if not enr:
            ...
            if self.border(behind_pos):
                ...
                if behind_block.get_output_activated_key((self.data["rotate"] + 2) % 4) != None:
                    if behind_block.logic_gate_active == 0:
                        self.data["activated"] = behind_block.data[behind_block.get_output_activated_key((self.data["rotate"] + 2) % 4)]
                else:
